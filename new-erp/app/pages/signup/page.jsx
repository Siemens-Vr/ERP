...
export default function Signup() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      {/* Container for Circles on the Left */}
      <div className="absolute inset-y-0 left-0 w-full md:w-1/2 h-full overflow-hidden flex justify-start items-end z-10">
      ...
      <form onSubmit={handleSignup} className="bg-white p-6 rounded-lg shadow-md w-96">
        ...
        <div className="mb-4">
          ...
          <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} required className="w-full px-4 py-2 border rounded-lg focus:ring-2" />
        </div>
        <div className="mb-4">
          <label className="block text-gray-700">Last Name</label>
          <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} required className="w-full px-4 py-2 border rounded-lg focus:ring-2" />
        </div>

        {/* Email */}
        <div className="mb-4">
          <label className="block text-gray-700">Email</label>
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-4 py-2 border rounded-lg focus:ring-2" />
        </div>
        ...
        <div className="mb-4 relative">
          ...
          <input
            ...
            className="w-full px-4 py-2 border rounded-lg focus:ring-2"
          />
          <span className="absolute top-10 right-3 cursor-pointer" onClick={() => setShowConfirmPassword(!showConfirmPassword)}>
        ...
        <button type="submit" disabled={!allValid || !passwordsMatch} className="w-full py-2 rounded-lg bg-blue-500 text-white font-semibold hover:bg-blue-600 disabled:bg-gray-400 disabled:cursor-not-allowed">
          Sign Up
        </button>
